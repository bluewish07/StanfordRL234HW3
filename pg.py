...
class PG(object):
  ...
  def build_policy_network_op(self, scope = "policy_network"):
    ...
    if self.discrete:
      ...
      with tf.variable_scope(scope):
        ...
        scale_mult = tf.ones(shape=[tf.shape(action_means)[0]])
        scale_mult = tf.Print(scale_mult, [scale_mult], message="scale multiplier")
        dist = tf.contrib.distributions.MultivariateNormalDiag(action_means, tf.exp(log_std), scale_identity_multiplier=scale_mult)
        sample = dist.sample()
  ...
  def add_baseline_op(self, scope = "baseline"):
    ...
    # TODO: output_activation = self.config.activation
    self.baseline = tf.squeeze(build_mlp(self.observation_placeholder, 1, scope=scope, output_activation=tf.nn.relu))
  ...
  def get_returns(self, paths):
    ...
    for path in paths:
      ...
      for idx, r in enumerate(reversed_rewards):
        if idx == 0:
          reversed_returns.append(r)
        else:
          G_t_plus_one = reversed_returns[idx-1]
          reversed_returns.append(r + self.config.gamma*G_t_plus_one)
  ...
  def calculate_advantage(self, returns, observations):
    ...
    if self.config.use_baseline:
      ...
      for idx, r in enumerate(returns):
        adv[idx] = r - baseline[idx]

    if self.config.normalize_advantage:
      mean, variance = tf.nn.moments(tf.convert_to_tensor(adv), axes=[0], keep_dims=True)
      adv_op = tf.nn.batch_normalization(adv, mean, variance, None, None, variance_epsilon=0.001)
      adv = self.sess.run(adv_op)
